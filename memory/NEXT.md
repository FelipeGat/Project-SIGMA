...
0. **BLOQUEIO ATIVO — [Proposal 6B (Planner Implementation)](../docs/releases/0006b-planner-implementation.md) aguarda aprovação explícita do Product Owner.** Toda a modelagem da 6A está aprovada e commitada; nenhum código do Planner pode ser escrito antes desse aval (princípio inegociável nº 12). **Revisão 2 (2026-08-11)**: o Product Owner decidiu unir 6B e 6C numa Release só — a proposta agora fecha o ciclo (`POST /intents` → Planner → `mission.planned` → `services/mission-worker` → Mission criada → `GET /missions/{id}`). Seis entregas, incluindo o primeiro consumo de evento pelo Mission Engine e um service novo. **Risco 2 é o que mais merece atenção**: uma Intent planejada com o worker fora do ar não vira Mission e ninguém percebe — o Planner é sem estado e o usuário já recebeu `202`. O cenário está no cenário 7 da Scenario Validation, para ser demonstrado, não escondido.
