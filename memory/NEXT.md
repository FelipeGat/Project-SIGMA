...
0. **Release 5.5 — Vertical Slice: COMPLETA.** O primeiro caminho ponta a ponta do SIGMA existe e está validado contra Docker real (ver [Validation Report 5.5](../docs/releases/0005.5-vertical-slice-validation-report.md)). **O achado de prioridade alta da 4.5 (`restart policy`) está resolvido e verificado** — não é mais pendência. **Próxima decisão do Product Owner**: seguir para a Release 6 (Planner Engine), que é o que faz uma Mission finalmente sair de `created`, ou endereçar antes uma das pendências abertas pela 5.5 (a mais relevante: `services/auth` ainda derruba `/health/live` quando o banco cai — mesmo problema que o gateway teve, corrigido só no gateway).
