...
0. **Release 6B — Planner Implementation: COMPLETA.** O ciclo fecha: `POST /intents` → Planner → `mission.planned` → `services/mission-worker` → Mission criada. Validado contra Docker real. **Próxima decisão do Product Owner** — três caminhos, em ordem de valor decrescente na minha leitura:
   - **(a) Release 7 — Intent Engine**: hoje quem chama informa o `kind` explicitamente; o Intent Engine é quem passa a inferi-lo de linguagem natural, completando a promessa declarativa. É o próximo da fila do ROADMAP.
   - **(b) Endereçar o `autonomyCeiling` fixo em `0`** (pendência 1 do Validation Report 6B): toda Mission criada por `POST /intents` para em `PendingApproval` no primeiro passo de nível ≥ 1, porque o Identity Engine não expõe o nível por User/Role (ADR-0068, aberto desde a 3B). É pequeno e destrava o uso real da rota nova.
   - **(c) Revisar os `requiredParameters` dos sete templates** (pendência 3): foram derivados de prosa, porque nenhum Playbook nomeia parâmetros. É o ponto mais frágil da 6B e não é trabalho de engenharia — é de quem é dono dos Playbooks.

0b. **Release 5.6 — Observability Baseline: COMPLETA.** ADR-0094 Aceita e implementada; health migrou para `packages/kernel/src/Http/`, `services/auth` ganhou os três endpoints, `gateway`/`auth` ganharam `healthcheck`. As duas pendências de observabilidade abertas pela 5.5 estão **fechadas**. **Próxima decisão do Product Owner**: a Release 6 (Planner Engine) é agora o caminho natural — é ela que faz uma Mission sair de `created`. Segue o Processo Oficial completo (ADR-0082): Research → Manifesto → Model → Lifecycle → Events → Contract → Proposal → aprovação → Implementation. Ver [Validation Report 5.6](../docs/releases/0005.6-observability-baseline-validation-report.md).
0b. **Release 5.5 — Vertical Slice: COMPLETA.** O primeiro caminho ponta a ponta do SIGMA existe e está validado contra Docker real (ver [Validation Report 5.5](../docs/releases/0005.5-vertical-slice-validation-report.md)). **O achado de prioridade alta da 4.5 (`restart policy`) está resolvido e verificado** — não é mais pendência. **Próxima decisão do Product Owner**: seguir para a Release 6 (Planner Engine), que é o que faz uma Mission finalmente sair de `created`, ou endereçar antes as duas pendências de observabilidade que a 5.5 abriu: **`services/auth` não tem endpoint de health nenhum** (ADR-0042 os define como padrão de todo processo deployável) e **nenhum serviço do SIGMA tem `healthcheck` no `docker-compose.yml`** — o gateway expõe os três endpoints desde a Release 2 e nada os consome. Corrigir os dois provavelmente exige subir `HealthEndpoints`/`BootFailureEndpoints` de `Sigma\Gateway\` para `packages/kernel`, o que é mudança entre módulos e portanto **pede uma ADR**, não um `fix` pontual.
...
4. ~~Aguardar confirmação para dar push dos commits das Releases 4/4.5/5~~ — **já pushados**: `origin/main` está em `628d7a1` (Release 5C), idêntico ao `main` local. Este item estava desatualizado e foi corrigido em 2026-08-11. A Release 5.5 vive na branch `Rossini` (`origin/Rossini`), ainda sem merge em `main`.
...
- ~~**PRIORIDADE ALTA — `docker-compose.yml` sem `restart policy`**~~ — **RESOLVIDO na Release 5.5**: `restart: on-failure` nos cinco serviços, verificado com evidência real (`docker compose stop redis` → worker de volta sozinho em ~6s).
