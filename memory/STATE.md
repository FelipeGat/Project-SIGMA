...
**Release 5A — Mission Research: Proposal apresentada, aguardando aprovação.** Bundle completo de pesquisa/modelagem entregue nesta rodada — [MISSION_MANIFESTO.md](../MISSION_MANIFESTO.md), [MISSION_MODEL.md](../MISSION_MODEL.md), [MISSION_LIFECYCLE.md](../MISSION_LIFECYCLE.md), [MISSION_EVENTS.md](../MISSION_EVENTS.md), `contracts/Mission.contract.yaml`, cinco ADRs (0089-0093). Achado real de pesquisa: `packages/README.md`/`packages/mission-engine/README.md` tinham uma dependência Composer (`mission-engine`→`planner-engine`) que contradizia [ADR-0031](../docs/adr/0031-ordem-runtime-vs-desenvolvimento.md) — corrigido. Decisões de domínio novas: Mission nasce do `Plan`, não da Intent (ADR-0089); aprovação é estado de primeira classe (ADR-0090); retry é histórico de Subtask, compensação é estado da Mission (ADR-0091); `Plan` é conceito próprio do Mission Engine, sem dependência de `planner-engine` (ADR-0092); `Mission.workspaceId` é opcional (ADR-0093). Nenhum código ainda — aguardando aprovação antes de 5B (Implementation). Ver [Decision Log 5A](../docs/releases/0005a-mission-research-decision-log.md).
...
**Aguardando aprovação da Proposal da Release 5A — Mission Research** ([docs/releases/0005a-mission-research.md](../docs/releases/0005a-mission-research.md)) — nenhum código de `packages/mission-engine` antes disso. Push do(s) commit(s) pendentes também aguardando confirmação explícita (mesma regra de sempre). Ver [NEXT.md](../memory/NEXT.md).
