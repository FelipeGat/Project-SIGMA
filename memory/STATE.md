...
**Release 5.6 — Observability Baseline: COMPLETA.** [Proposal 5.6](../docs/releases/0005.6-observability-baseline.md) + [ADR-0094](../docs/adr/0094-health-endpoints-pertencem-ao-kernel.md) (Aceita) implementadas. `HealthEndpoints`/`BootFailureEndpoints` migraram de `Sigma\Gateway\` para `packages/kernel/src/Http/` — health é comportamento de Module, não de service; `services/auth` ganhou os três endpoints de ADR-0042 (não tinha nenhum desde a 3B); `gateway`/`auth` ganharam `healthcheck` no compose apontando para `/health/ready`. **Primeira vez no projeto em que o estado interno de um Module influencia uma decisão do orquestrador**: com `docker compose stop mariadb`, ambos viram `unhealthy` em ~25s sem nenhum processo morrer, `live` segue `200`, `ready` responde `503`; ao religar, voltam a `healthy` em **4s**. **268 testes, 641 assertions, 0 pulados** (eram 265). Achado: a entrega 4 da Proposal (`depends_on: service_healthy`) partia de premissa errada — todos os `depends_on` já usavam essa condição; registrado no Decision Log em vez de gerar trabalho inventado. `gateway.unavailable` virou `service.unavailable` (o código nomeava um service específico numa classe agora compartilhada). Ver [Decision Log 5.6](../docs/releases/0005.6-observability-baseline-decision-log.md)/[Validation Report 5.6](../docs/releases/0005.6-observability-baseline-validation-report.md).
...
- ~~`services/auth` sem endpoint de health~~ e ~~nenhum `healthcheck` no compose~~ — **ambos RESOLVIDOS na Release 5.6**.
- **`memory-worker` continua sem healthcheck** — exceção consciente de ADR-0094 (não escuta HTTP); sua saúde real (está consumindo eventos?) segue não observável.
- **Metrics, Tracing e Audit** (os outros três pilares de TELEMETRY.md) continuam sem implementação — Release 23.
