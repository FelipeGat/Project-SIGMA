...
- Adiciona complexidade de configuração (quem define o nível de cada User/Role, quem define o nível mínimo de cada Capability) — trabalho a ser detalhado no épico correspondente (Release 8 — Agent Engine e Release 7 — Skill Engine, onde autonomia é de fato aplicada), não nesta ADR.
